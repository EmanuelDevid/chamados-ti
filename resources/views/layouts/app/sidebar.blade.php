<aside class="w-64 bg-slate-900 text-slate-200 flex flex-col h-screen sticky top-0 border-r border-slate-800 flex-shrink-0">
    ...
    <div class="flex-1 overflow-y-auto custom-scrollbar">
        <nav class="p-4 space-y-1">
            ...
            {{-- Seção Gestão TI (Restrita a Administradores) --}}
            @if(auth()->user()->is_admin)
            <div class="px-3 text-[11px] font-semibold uppercase tracking-wider text-slate-400 mt-6 mb-2">
                Gestão TI
            </div>

            {{-- Item Fila de Atendimento com Submenu via Alpine.js --}}
            <div x-data="{ open: {{ request()->routeIs('tickets.index') || $isQueueDetails ? 'true' : 'false' }} }">

                {{-- Botão Principal da Fila --}}
                <button @click="open = !open"
                    class="w-full flex items-center justify-between px-3 py-2.5 rounded-md text-sm font-medium transition text-slate-300 hover:bg-slate-800 hover:text-white">
                    <div class="flex items-center gap-3 overflow-hidden">
                        <svg width="20" height="20" class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                {{-- Submenu de Status --}}
                <div x-show="open" x-collapse class="mt-1 pl-7 space-y-1">

                    {{-- Todos os Chamados --}}
                    <a href="{{ route('tickets.index') }}" wire:navigate
                        class="block px-3 py-1.5 rounded-md text-xs font-medium transition {{ request()->fullUrl() === route('tickets.index') ? 'bg-blue-600 text-white' : 'text-slate-400 hover:bg-slate-800 hover:text-white' }}">
                        Todos os Chamados
                    </a>

                    {{-- Novos --}}
                    <a href="{{ route('tickets.index', ['statusFilter' => 'novo']) }}" wire:navigate
                        class="flex items-center justify-between px-3 py-1.5 rounded-md text-xs font-medium transition {{ request()->query('statusFilter') === 'novo' ? 'bg-blue-600 text-white' : 'text-slate-400 hover:bg-slate-800 hover:text-white' }}">
                        <span>Novos</span>
                        @php $countNovo = \App\Models\Ticket::where('status', 'novo')->count(); @endphp
                        @if($countNovo > 0)
                        <span class="bg-red-500 text-white text-[10px] font-bold px-1.5 py-0.2 rounded-full animate-pulse">
                            {{ $countNovo }}
                        </span>
                        @endif
                    </a>
                    ...
                    {{-- Em Atendimento --}}
                    <a href="{{ route('tickets.index', ['statusFilter' => 'em_atendimento']) }}" wire:navigate
                        class="block px-3 py-1.5 rounded-md text-xs font-medium transition {{ request()->query('statusFilter') === 'em_atendimento' ? 'bg-blue-600 text-white' : 'text-slate-400 hover:bg-slate-800 hover:text-white' }}">
                        Em Atendimento
                    </a>

                    {{-- Aguardando Usuário --}}
                    <a href="{{ route('tickets.index', ['statusFilter' => 'aguardando_usuario']) }}" wire:navigate
                        class="block px-3 py-1.5 rounded-md text-xs font-medium transition {{ request()->query('statusFilter') === 'aguardando_usuario' ? 'bg-blue-600 text-white' : 'text-slate-400 hover:bg-slate-800 hover:text-white' }}">
                        Aguardando Usuário
                    </a>

                    {{-- Concluídos / Resolvidos --}}
                    <a href="{{ route('tickets.index', ['statusFilter' => 'concluido']) }}" wire:navigate
                        class="block px-3 py-1.5 rounded-md text-xs font-medium transition {{ request()->query('statusFilter') === 'concluido' ? 'bg-blue-600 text-white' : 'text-slate-400 hover:bg-slate-800 hover:text-white' }}">
                        Concluídos
                    </a>
